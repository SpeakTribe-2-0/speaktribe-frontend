...
const YorubaWords = () => {
  ...
  const { saveProgress, languageProgress, loadAllProgress } =
  ...
  const dailyWords = VOCAB.slice(
    dayIndex * wordsPerDay,
    dayIndex * wordsPerDay + wordsPerDay
  );
  ...
  useEffect(() => {
    if (showQuiz && !quizQuestions.length) {
      const currentBatch = VOCAB.slice(
        dayIndex * wordsPerDay,
        dayIndex * wordsPerDay + wordsPerDay
      );
          ...
          {!showQuiz ? (
            ...
            >
              ...
              <div className="text-center">
                <h1 className="text-4xl font-bold text-gray-800 mb-4 flex items-center justify-center gap-3 max-[621px]:text-2xl">
                  <BookOpen className="text-[#009688]" size={40} />
                  Yoruba Words — Days {dayIndex + 1} / {totalDays}
                </h1>
                <div className="mb-6">
                  <div className="flex justify-between text-sm text-gray-600 mb-2 max-w-md mx-auto">
                    <span>Words Progress</span>
                    <span>
                      {Math.round(((dayIndex + 1) / totalDays) * 100)}%
                    </span>
                  </div>
                  <div className="w-full max-w-md mx-auto bg-gray-200 rounded-full h-3">
                    <motion.div
                      initial={{ width: 0 }}
                      animate={{
                        width: `${((dayIndex + 1) / totalDays) * 100}%`,
                      }}
                  ...
                  <div className="p-8">
                    ...
                    <div className="mb-6">
                      <div className="flex justify-between text-sm text-gray-600 mb-2">
                        ...
                        <span className="text-white">
                          {currentCardIndex + 1} / {dailyWords.length}
                        </span>
                      </div>
                      <div className="w-full bg-gray-200 rounded-full h-2">
                        <motion.div
                          ...
                          animate={{
                            width: `${
                              ((currentCardIndex + 1) / dailyWords.length) * 100
                            }%`,
                          }}
                    ...
                    <div className="text-center mb-6">
                      <div className="flex items-center justify-center gap-4 mb-4">
                        <h3 className="text-[#009688] font-bold text-4xl">
                          {currentWord.word}
                        </h3>
                        <motion.button
                    ...
                    {/* Navigation */}
                    <div className="flex justify-between items-center mt-8 max-[621px]:gap-3">
                      <motion.button
                        ...
                        className={`flex items-center gap-2 px-6 py-3 rounded-xl font-semibold transition-all max-[621px]:px-1 max-[621px]:py-2 max-[621px]:gap-1 max-[621px]:text-sm ${
                          currentCardIndex === 0
                      ...
                      {currentCardIndex === dailyWords.length - 1 ? (
                        <motion.button
                          ...
                          whileHover={{ scale: 1.05 }}
                          whileTap={{ scale: 0.95 }}
                          className="flex items-center gap-3 px-8 py-4 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-xl shadow-lg hover:shadow-xl transition-all font-bold text-lg max-[621px]:px-2 max-[621px]:py-2 max-[621px]:gap-1 max-[621px]:text-sm"
                        >
                          <Trophy size={24} />
                          Take Quiz{" "}
                          <span className="max-[621px]:hidden">Challenge!</span>
                        </motion.button>
                      ) : (
                        <motion.button
                          whileHover={{ scale: 1.05 }}
                          ...
                          onClick={() =>
                            setCurrentCardIndex((prev) =>
                              Math.min(prev + 1, dailyWords.length - 1)
                            )
                          }
                          className="flex items-center gap-2 px-6 py-3 bg-[#009688] text-white rounded-xl font-semibold shadow-lg hover:shadow-xl transition-all hover:bg-[#00796b] max-[621px]:px-2 max-[621px]:py-2 max-[621px]:gap-1 max-[621px]:text-[15px]"
                        >
                          Next <span>Word</span>
            ...
            >
              ...
              <div className="text-center mb-8">
                ...
                <p className="text-gray-600">
                  Test your knowledge of today's words
                </p>
              </div>
              ...
              <div className="space-y-6">
                {quizQuestions.map((q, i) => {
                  ...
                  const isWrong =
                    submitted && quizAnswers[i] && quizAnswers[i] !== q.correct;
                          ...
                          <div
                            className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold text-white shadow-lg ${
                              isCorrect
                                ...
                                : isWrong
                                ? "bg-red-500"
                                : "bg-gradient-to-r from-[#009688] to-emerald-600"
                            }`}
                          >
                            {i + 1}
                          </div>
                          <div>
                            <h4 className="text-xl font-bold text-[#009688]">
                              {q.word}
                            </h4>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          {q.options.map((opt, idx) => {
                            ...
                            const isWrongSelection =
                              submitted && isSelected && opt !== q.correct;

                            return (
                              <motion.label
                                key={idx}
                                whileHover={
                                  !submitted ? { scale: 1.02, y: -2 } : {}
                                }
                      ...
                      {allQuizCorrect ? (
                        ...
                        >
                          ...
                          {dayIndex === totalDays - 1
                            ? "🎊 Complete Words & Go to Sentences!"
                            : "🌟 Unlock Next Day!"}
                        </motion.button>
                      ) : (
                        <motion.button
